% task3_main.m
% Program to simulate and plot two parallel link network results
clear;
clc;
close all;
...
for k_idx = 1:length(K_values)
    ...
    for p_idx = 1:length(p_values)
        p = p_values(p_idx);
        % Run simulation
        simulated_results(p_idx) = runTwoParallelLinkSim(K, p, N);
    end
    ...
    title(ax_individual, sprintf('Two Parallel Link Network Results (K = %d)', K), ...
        'FontSize', 14);
...
title(ax_combined, 'Two Parallel Link Network Results - All K Values', ...
    'FontSize', 14);
